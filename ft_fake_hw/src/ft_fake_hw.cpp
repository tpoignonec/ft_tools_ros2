...
std::vector<hardware_interface::StateInterface> FtFakeHw::export_state_interfaces()
{
  ...
  for (auto i = 0u; i < info_.sensors.size(); i++)
  {
    ...
    RCLCPP_INFO(node_->get_logger(), "sensor.state_interfaces.size() = %u", sensor.state_interfaces.size());
...
hardware_interface::return_type FtFakeHw::read(const rclcpp::Time& /*time*/, const rclcpp::Duration& /*period*/)
{
  if (rclcpp::ok())
  {
